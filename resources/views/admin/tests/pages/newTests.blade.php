				...
				@if($tests->count())
					<div class="text-center">
						<div class="btn-group p-1">
							<a href="{{ route('admin.tests.newTests') }}" class="{{ Request::is('admin/tests/newTests') ? "btn-secondary": "btn-primary" }} btn btn-sm">All</a>
							@foreach($letters as $value)
								<a href="{{ route('admin.tests.newTests', $value) }}" class="{{ Request::is('admin/tests/newTests/'.$value) ? "btn-secondary": "btn-primary" }} btn btn-sm">{{ strtoupper($value) }}</a>
							@endforeach
						</div>
					</div>
				@endif
