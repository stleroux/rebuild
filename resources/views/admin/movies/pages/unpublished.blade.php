         ...
         <div class="card-header section_header p-2">
            ...
            <span class="float-right">
               <div class="btn-group">
                  ...
                  @include('admin.movies.buttons.publishAll', ['size'=>'xs'])
         ...
         @if($movies->count())
            <div class="text-center">
               <div class="btn-group p-1">
                  <a href="{{ route('admin.movies.unpublished') }}" class="{{ Request::is('admin/movies/unpublished') ? "btn-secondary": "btn-primary" }} btn btn-sm">All</a>
                  @foreach($letters as $value)
                     <a href="{{ route('admin.movies.unpublished', $value) }}" class="{{ Request::is('admin/movies/unpublished/'.$value) ? "btn-secondary": "btn-primary" }} btn btn-sm">{{ strtoupper($value) }}</a>
                  @endforeach
               </div>
            </div>
         @endif
