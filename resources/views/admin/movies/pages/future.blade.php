         ...
         @if($movies->count())
            <div class="text-center">
               <div class="btn-group p-1">
                  <a href="{{ route('admin.movies.future') }}" class="{{ Request::is('admin/movies/future') ? "btn-secondary": "btn-primary" }} btn btn-sm">All</a>
                  @foreach($letters as $value)
                     <a href="{{ route('admin.movies.future', $value) }}" class="{{ Request::is('admin/movies/future/'.$value) ? "btn-secondary": "btn-primary" }} btn btn-sm">{{ strtoupper($value) }}</a>
                  @endforeach
               </div>
            </div>
         @endif
               ...
               <table id="datatable" class="table table-hover table-sm searchHighlight">
                  <thead>
                     <tr>
                        ...
                        <th class="hidden-sm hidden-xs">Publish On</th>
