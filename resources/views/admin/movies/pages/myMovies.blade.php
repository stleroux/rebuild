         ...
         @if($movies->count())
            <div class="text-center">
               <div class="btn-group p-1">
                  <a href="{{ route('admin.movies.myMovies') }}" class="{{ Request::is('admin/movies/myMovies') ? "btn-secondary": "btn-primary" }} btn btn-sm">All</a>
                  @foreach($letters as $value)
                     <a href="{{ route('admin.movies.myMovies', $value) }}" class="{{ Request::is('admin/movies/myMovies/'.$value) ? "btn-secondary": "btn-primary" }} btn btn-sm">{{ strtoupper($value) }}</a>
                  @endforeach
               </div>
            </div>
         @endif
