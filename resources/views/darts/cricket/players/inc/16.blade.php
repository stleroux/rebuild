<tr>
   <td class="align-middle">
      {!! Form::open(['route'=>'darts.cricket.players.store']) !!}
         ...
         <button type="submit" name="score" value="16" class="btn btn-sm btn-success" {{ ($game->status == 'Completed') ? 'disabled' : '' }}>
   ...
   <td class="align-middle">
      {!! Form::open(['route'=>'darts.cricket.players.store']) !!}
         <input type="hidden" name="user_id" value="1">
         <input type="hidden" name="game_id" value="{{ $game->id }}">
         <button type="submit" name="score" value="-16" class="btn btn-sm btn-danger" {{ ($game->status == 'Completed') ? 'disabled' : '' }}>
