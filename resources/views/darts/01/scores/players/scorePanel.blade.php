         ...
         {!! Form::open(['route' => 'darts.01.scores.players.store']) !!}
            ...
            {{-- <div class="col-xs-12 border"> --}}
               {{-- <div class="btn-group btn-group-vertical {{ $errors->has('user_id') ? 'has-error' : '' }}" data-toggle="buttons"> --}}
                  ...
                  {{-- <br /> --}}
                     {{-- Remaining Score : --}}
                     {{ Form::hidden('remainingScore', ($game->type - zeroOnePlayerScore($game->id, $user->user_id)->sum('score')), ['size'=>2]) }}
                     {{-- <br /> --}}
                     ...
                     <label class="col p-2 m-0 btn btn-primary {{ ($user->shooting_order == zeroOneNextShot($game->id)) ? 'active':'' }}">
                        ...
                        {{-- @if(zeroOneNextShot($game) == $user->shooting_order) --}}
                           {{ Form::radio('user_id', $user->user_id , ($user->shooting_order == zeroOneNextShot($game->id)) ?? true) }}
                        {{-- @endif --}}
            ...
            <div class="container pt-2">
               <div class="row justify-content-md-center">
                  <div class="col-xs-12 col-sm-6">
                     @if($game->type - zeroOnePlayerScore($game->id, $user->user_id)->sum('score') <= 0)
